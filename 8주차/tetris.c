...
int modified_recommend(RecNode *root){
	...
	// user code
	for (rotate=0; rotate<NUM_OF_ROTATE; rotate++){
		if (rotate >= 1 && nextBlock[root->lv]==4) break; // 2x2 정사각형 블록은 회전이 1회만 필요
		else if (rotate >= 2 && (nextBlock[root->lv] == 0 || nextBlock[root->lv] >= 5)) break; // 회전이 2회만 필요한 블록
		for (Xpos=-1; Xpos<WIDTH; Xpos++){
			Ypos=0;
			while (Ypos<HEIGHT && CheckToMove(root->f, nextBlock[root->lv], rotate, Ypos, Xpos)) Ypos++;
			Ypos--;
...
void recommendedPlay(){
	int command, spacesize;
	clear();
	act.sa_handler = recommended_drop;
	sigaction(SIGALRM,&act,&oact);
	InitTetris();
	start = time(NULL);
	do{
	...
	printw("Total played time: %lf seconds\n", duration);
	printw("Total used space: %ld bytes\n", spacesize);
	printw("\nTime efficiency: %lf\n", (double)score / duration);
	printw("Space efficiency: %lf\n", (double)score / (double)spacesize);
	getch();
	//newRank(score);
}
